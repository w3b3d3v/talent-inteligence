...
async def processMessagesOnChannel(
    channel: str, msg_limit: int, after: datetime.datetime = None
) -> List[str]:
    messages = [
        (message.content, message.author.id, message.created_at)
        async for message in channel.history(
            limit=msg_limit, oldest_first=True, after=after
        )
    ]
...
async def processSingleMessage(
    message_content: str, author_id: str
) -> List[Dict[str, Any]]:
    matcher = Matcher(jobs=JOBS, techs=TECHS, names=NAME, uf=UF)
...
async def on_message(message):
    ...
    if message.content.startswith("!model"):
        ...
        if command == "processAll":
            if len(args) != 5:
                await message.channel.send(
                    "Comando inválido. Use: `!model processAll <guild_id> <channel_id> <num_messages>`"
                )
                return
            ...
            try:
                ...
                await message.channel.send(
                    "Não foi possível encontrar o servidor ou canal com os IDs fornecidos."
                )
                return

            last_date = dateStore.load_last_date()
            predictions = await processMessagesOnChannel(
                channel, num_messages, last_date
            )
            ...
            await message.channel.send(
                f"Estamos em {server_count} servidores:\n{server_names}"
            )

        elif command == "permissions":
            if len(args) != 3:
                await message.channel.send(
                    "Comando inválido. Use: `!model permissions <guild_id>`"
                )
                return
            ...
            if len(args) != 3:
                await message.channel.send(
                    "Comando inválido. Use: `!model grantRole <role_id>`"
                )
                return

            role_id = args[2]
            try:
                await grant_role_to_users(
                    client=client, filename="users.csv", role_id=role_id
                )
        ...
        try:
            predictions = await processSingleMessage(
                message_content=message.content, author_id=str(message.author.id)
            )
            store_predictions(predictions=predictions)
        except Exception as e:
            print(e)
